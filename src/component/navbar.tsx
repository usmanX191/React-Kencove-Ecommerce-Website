...
import { faTimes ,  faUser, faCartShopping, faChevronDown, faChevronUp, faSearch} from '@fortawesome/free-solid-svg-icons';
...
console.log(`Window width: ${windowWidth}px`);
  ...
  const [isSearchPopupVisible, setSearchPopupVisibility] = useState<Boolean>(false);
    ...
    <>
      ...
      <div className="w-full sm:sticky top-0 bg-blue-800 sm:h-40 xs:h-15">
        <div className="flex">
          ...
          <div className='xs:block sm:hidden xs:px-4 xs:py-8 xs:-mt-1'><button type='button'><FontAwesomeIcon icon={faSearch} style={{ color: '#ffffff', fontSize: '18px' }} onClick={showSearchPopup}/></button>
          </div>
          ...
          <p className="xs:text-xs xs:font-semibold xs:px-0 xs:py-8 inline-block 2xl:ml-5 2xl:px-6 2xl:py-10 lg:py-10 lg:px-3 2xl:text-lg lg:text-base text-white font-bold cursor-pointer relative" onMouseEnter={showSignInPopup} onMouseLeave={hideSignInPopup}>
          <FontAwesomeIcon className='xs:inline-block 2xl:-mr-0 xs:mr-0.5' icon={faUser} style={{ color: '#ffffff' }} />
          <button
            type="button"
            className="2xl:px-1 lg:px-1 font-bold text-white hover:underline hover:text-white xs:inline-block"
          >
          Sign in
          {isSignInPopupVisible ? <FontAwesomeIcon className='xs:hidden sm:block sm:inline-block px-1' icon={faChevronUp} style={{color: "#ffffff",}} /> : <FontAwesomeIcon className='xs:hidden sm:block sm:inline-block px-1' icon={faChevronDown} style={{color: "#ffffff",}} />}
          </button>
          {isSignInPopupVisible && (
              <div className="absolute top-full h-70 xs:w-auto left-0 xs:-mt-1 sm:ml-1 sm:-mt-8 bg-gray-300 text-black rounded-lg">
                <button className="w-full text-center hover:bg-gray-200 bg-blue-500 text-white sm:font-semibold sm:text-lg xs:text-xs py-4 px-5" onClick={hideSignInPopup}>
                  SignIn
                </button>
                <button className="w-full text-center p-2 hover:bg-gray-500 xs:font-normal" onClick={hideSignInPopup}>
                  Create Account
                </button>
              </div>
            )}
        </p>
          <p className="xs:text-xs xs:font-semibold xs:px-2 xs:py-8 xs:ml-2 inline-block 2xl:ml-5 2xl:py-10 lg:py-10 lg:px-3 2xl:text-lg text-white font-bold cursor-pointer relative" onMouseEnter={showCartPopup} onMouseLeave={hideCartPopup}>
            ...
            <button
              ...
              // window.location.href = 'cart';
                ...
                Cart
              {isCartPopupVisible ? <FontAwesomeIcon className='xs:hidden sm:block sm:inline-block px-1' icon={faChevronUp} style={{color: "#ffffff",}} /> : <FontAwesomeIcon className='xs:hidden sm:block sm:inline-block px-1' icon={faChevronDown} style={{color: "#ffffff",}} />}  
            </button>
            {isCartPopupVisible && (
        ...
        <div className="xs:hidden sm:block flex 2xl:-mt-1">
          ...
          <p className="px-5 py-3.5 text-lg text-white font-bold cursor-pointer relative inline-block" onMouseEnter={showProductPopup} onMouseLeave={hideProductPopup}>
            <button
              type="button"
              className="font-bold text-white hover:underline hover:text-white"
              onClick={() => {
      ...
      {isSearchPopupVisible && (
            ...
            >
              ...
              <button className='absolute xs:mt-10 xs:ml-16' onClick={showSearchPopup}>
